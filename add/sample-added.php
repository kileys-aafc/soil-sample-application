...
<?php
    if(isset($_POST['add-sample'])){
        ...
        //------Archive info from $_POST[]--------

        $arch_year = $_POST['arch_year'];
            ...
            if($affected_rows_sample_info == 1){         
                ...
                if($affected_rows_physical == 1){
                    ...
                    if($affected_rows_chemical == 1){
                        ...
                        $query_insert_archive = "INSERT INTO archive (`sample_id`, `arch_year`, `section`, `arch_col`, `arch_row`, `box_id`, `weight`) VALUES (?,?,?,?,?,?,?)";
                
                        $stmt_insert_archive = mysqli_prepare($dbc, $query_insert_archive);
                        
                        //-- Check if stmt is false
                        if($stmt_insert_archive == false) {
                        die("<pre>".mysqli_error($dbc).PHP_EOL.$query_insert_archive."</pre>");
                        }
                
                        mysqli_stmt_bind_param($stmt_insert_archive, "iissssd", $sample_id, $arch_year, $section, $column, $row, $box_id, $weight);
                        mysqli_stmt_execute($stmt_insert_archive);
                        $affected_rows_archive = mysqli_stmt_affected_rows($stmt_insert_archive);  
                    
                        if($affected_rows_archive == 1){
                                ...
                                mysqli_stmt_close($stmt_insert_archive);        
