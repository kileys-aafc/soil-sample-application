...
<div class="container">
	...
	<?php        
		...
		$year = $_POST['year'];
		if($_POST['date'] == ""){$date = "NULL";}else{$input_date = $_POST['date']; $date='"'.$input_date.'"';}
		...
		$arch_col = strtoupper($_POST['arch_col']);
		$arch_row = $_POST['arch_row'];
		$box_id = $_POST['box_id'];
		$weight = $_POST['weight'];
...
//-----------------Archive Update Query------------
        $query_update .= "INSERT INTO archive (sample_id, arch_year, section, arch_col, arch_row, box_id, weight) VALUES($sample_id, $arch_year, '$section', '$arch_col', $arch_row, $box_id, $weight) 
        ON DUPLICATE KEY UPDATE arch_year=$arch_year, section='$section', arch_col='$arch_col', arch_row=$arch_row, box_id=$box_id, weight=$weight;";
